...
#Inorder traversal? O(n) time and O(h) space
#Top-to-bottom? O(h) time, which is O(log(n)) if balanced or O(n) if unbalanced
...
def find_second_largest(root):
    if (root is None or (root.left is None and root.right is None)):
        raise ValueError('At least 2 nodes are needed')
    #We are at largest and it has a left subtree
    if root.left and not root.right:
        return find_largest(root.left)
    #We are at parent of largest and largest has no subtrees
