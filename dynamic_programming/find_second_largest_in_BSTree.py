"""
Find second largest value in a BST
"""
import unittest
...
def find_second_largest(root):
    if(root is None or (root.left is None and root.right is None)):
        raise ValueError('BST has at least 2 nodes')
    cur = root;
    while cur:
        #cur is the largest and has left subtree
        #we reached rightmost element, which is the largest
        if cur.left and not cur.right:
            return find_largest(cur.left)
        #cur is parent of largest and largest has no children
